...
const BELL_SIZES = ["regular", "big"] as const satisfies readonly LoomiSize[];
...
export class LoomiBell extends LoomiElement {
  ...
  static readonly supportedSizes = { size: BELL_SIZES } satisfies LoomiSizeSupport;

  @property() color: LoomiColor = "primary" as LoomiColor;
  /** Icon size: `regular` | `big`. */
  ...
  override render(): TemplateResult {
    return html`<span class="loomi-bell size-${resolveLoomiSize(this.size, BELL_SIZES)} ${this.invert ? "invert" : ""}" style=${accentVars(this.color)}>
